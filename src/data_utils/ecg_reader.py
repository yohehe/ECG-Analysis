...
## DATASETのことを考えていかに設定変更
def load_ecg_metadata_by_id(ecg_id: int, train_df: pd.DataFrame) -> dict:
    row = train_df[train_df["id"] == ecg_id]
    if row.empty:
        raise ValueError(f"ID {ecg_id} が train_df に存在しません")
    ...
    return {
        ...
        "duration_sec": int(row["sig_len"]) / float(row["fs"]),
        "csv_path": row["ecg_csv_path"],
        "png_image_paths": row["png_image_paths"]
    }

def extract_lead_features(ecg_dict: dict) -> dict:
    csv_path = ecg_dict["csv_path"]
    ecg_df = pd.read_csv(csv_path)
